...
namespace Xania.CoreUI.Drive
{
    public class DocumentObjectStore<TModel> : IObjectStore<TModel>
    {
        ...
        private readonly string _documentFolder = typeof(TModel).FullName.ToLowerInvariant();
        ...
        {
            foreach (var resourceId in _documentStore.List(_documentFolder))
                yield return Deserialize(_documentStore.OpenRead(_documentFolder, resourceId));
        }

        public Task<TModel> AddAsync(TModel model)
        {
            var resourceId = GetResourceId(model);
            Serialize(model, _documentStore.OpenWrite(_documentFolder, resourceId));
        ...
        public async Task DeleteAsync(Expression<Func<TModel, bool>> condition)
        {
            ...
            foreach (var resourceId in _documentStore.List(_documentFolder))
            {
                var model = Deserialize(_documentStore.OpenRead(_documentFolder, resourceId));
                if (compiled(model))
                    await _documentStore.DeleteAsync(_documentFolder, resourceId);
            }
        }
